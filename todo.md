...
- [ ] `firstReal`-Fallback entfernen; `selectedVariables` bleibt leer, wenn der
      Benutzer nichts über "add to Oszi" angefordert hat (`pendingProbes`).
- [ ] Bestehende Probes (`kept`) und `pendingProbes` weiterhin übernehmen.
- [ ] Test: erste Simulation ohne Probes → leere Auswahl; mit `addProbe` vorher
...
- [ ] Netz-Id des Net-Labels auflösen (`comp.ports[0].netId`) und `vExpr` via
      `netVoltageExpr` ins Menü legen.
- [ ] Menüeinträge "V(net) im Oszi anzeigen" + "Datenpunkt: Potential V(net)".
- [ ] Test: Menü-Expression für ein Net-Label ist das `V(<netname>)` seines Netzes.
...
- [ ] `renameNetInProbe` um die Form `V(a,b)` erweitern (beide Argumente).
- [ ] Prüfen, dass `renameNet` → `renameNetVariable` + `renameTraceNet` alle
      Senken erreicht: Traces, Expressions, Farben, Panel-Zuordnung, dataFlags.
- [ ] Test: Rename zieht `V(a)`, `V(a,b)` und `-V(a)` mit; `I(...)` bleibt
      unangetastet.
...
- [ ] Pin-zu-Pin-Andocken ergänzen: deckungsgleiche Pins anderer Bauteile finden
      und direkt verdrahten (Leitungs-Tap bleibt Vorrang/unverändert).
- [ ] Kein Self-Connect, keine Doppelkante, wenn die Verbindung schon existiert.
- [ ] Test: Net-Label auf einen Bauteil-Pin gesetzt → gemeinsames Netz im
      Netlist-Rebuild; Netzname des Labels landet am Bauteil-Pin.

## Abschluss
- [ ] `npm run check` + `npm run test:editor` grün.
- [ ] Vier getrennte Commits.
